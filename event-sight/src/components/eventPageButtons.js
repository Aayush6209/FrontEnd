...
const EventPageButtons = (props)=>{
    ...
    return <div className="EventPageButtonsDiv">
        <Row>
          ...
          <Button className="EventPageButtonsUpper" color="danger" onClick={()=>{props.cancelRegistration(props.sid, props.token, props.event.id)}}><MdEvent size="23px"/> Unregister</Button> : 
          <Button className="EventPageButtonsUpper" outline color="success" onClick={()=>{props.registerEvent(props.event.id, props.sid, props.token)}}><MdEvent size="23px"/> Register</Button>}</Col> */}
